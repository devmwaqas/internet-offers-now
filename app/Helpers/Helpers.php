...
if (!function_exists('providers_not_in_location')) {
	function providers_not_in_location($zipcode)
	{
		$query = DB::table('providers')
			->whereNotIn('id', function ($query) use ($zipcode) {
				$query->select('provider_id')->from('offers')->where('zip', $zipcode);
			});
		return $query->get();
	}
}
